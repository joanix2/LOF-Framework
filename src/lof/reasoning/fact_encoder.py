...
class FactEncoder:
    ...
    def encode(self, silver: SilverGraph) -> list[Fact]:
        ...
        for e in silver.entities.values():
            ...
            facts.append(Fact(
                predicate="entity", args=(e.name.lower(),), status=status,
                bronze_ids=[p.bronze_id for p in e.provenance if p.bronze_id],
            ))
            facts.append(Fact(
                predicate="entity_type", args=(e.name.lower(), e.type), status=status,
            ))
            for k, v in e.attributes.items():
                facts.append(Fact(
                    predicate=k, args=(e.name.lower(), str(v).lower()), status=status,
                ))
        ...
        for c in silver.claims.values():
            ...
            args = (c.subject.lower(), predicate, c.object.lower()) if is_norm else (c.subject.lower(), c.object.lower())  # noqa: E501
            facts.append(Fact(
                predicate=pred, args=args, status=status, confidence=c.confidence,
        ...
        for r in silver.relations.values():
            ...
            facts.append(Fact(
                predicate=r.kind, args=(r.source.lower(), r.target.lower()),
                status=status, confidence=r.confidence,
