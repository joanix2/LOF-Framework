...
class BenchmarkRunner:
    ...
    def run_all(self, level: int = -1, category: str | None = None) -> BenchmarkReport:
        ...
        if results:
            report.composite_score = sum(
                r.bronze_score + r.silver_score + r.gold_score
                + r.diagnostic_score + r.generation_score
                for r in results
            ) / (len(results) * 5.0)
        return report
    ...
    def _test_silver(self, scenario: ScenarioDefinition, errors: list[str]) -> float:
        ...
        if scenario.silver.entity_count > 0 or scenario.silver.claim_count > 0:
            ...
            if scenario.silver.entity_count > 0 and len(entities) < scenario.silver.entity_count:
                errors.append(f"Silver: expected>={scenario.silver.entity_count} e, got {len(entities)}")  # noqa: E501
    ...
    def _test_smt(self, scenario: ScenarioDefinition, errors: list[str], compiler: Compiler) -> float:  # noqa: E501
        try:
            ig = InstanceGraph()
    ...
    def _test_generation(self, scenario: ScenarioDefinition, errors: list[str], compiler: Compiler) -> float:  # noqa: E501
        try:
...
class ScenarioState:
    ...
    def _duplicate_output(self):
        ...
        self.compiler.registry.register_type(t1)
        self.compiler.registry.register_type(t2)
        self.compiler.registry.register_instance(InstanceDefinition(
            id="dup-a", type="dup-output-a", values={"name": "A"}
        ))
        self.compiler.registry.register_instance(InstanceDefinition(
            id="dup-b", type="dup-output-b", values={"name": "B"}
        ))
