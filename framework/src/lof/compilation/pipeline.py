...
class Pipeline:
    ...
    def process_instance(self, instance: InstanceDefinition) -> tuple[Artifact | None, list[str]]:
        ...
        patches = [
            p for pid in instance.patches
            if (p := self.registry.get_patch(pid)) is not None
        ]
        ...
        artifact = Artifact(
            instance=instance.id, type=td.id,
            template=template_path,
            patches=[p.id for p in patches],
            output=out or "", hash=compute_hash(source or ""),
    ...
    def _resolve_output_path(self, td: TypeDefinition, ctx: dict[str, Any],
                              instance: InstanceDefinition) -> str | None:
