...
class Z3SolverAdapter:
    def validate(self, context: "ConstraintContext",
                 constraints: Sequence[ConstraintDefinition]) -> SolverResult:
        solver = z3.Solver()
        ...
        if check == z3.unsat:
            ...
            core_as = [a for a in all_assertions if a.name in core_names] if core_names else all_assertions  # noqa: E501
            violations = [
                ConstraintViolation(
                ...
                ConstraintViolation(
                    constraint_id="root", code="UNSAT",
                    message="Model is unsatisfiable.", unsat_core=core_names,
                )
            ]
