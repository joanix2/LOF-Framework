        ...
        for inst in context.registry.instances.values():
            ...
            if not found:
                assertions.append(NamedAssertion(
                    ...
                    metadata={
                        "message": f"'{inst.id}' missing {prop}=True in {nested_in or 'values'}",
                        "hint": f"Add '{prop}: true' to the appropriate location.",
                    },
                ))

        return assertions or [
            NamedAssertion(name=f"required_{prop}_ok", expression=z3.BoolVal(True), constraint_id=definition.id)  # noqa: E501
        ]
