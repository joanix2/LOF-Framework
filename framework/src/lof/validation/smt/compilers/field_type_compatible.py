...
_TYPE_MAP = {
    "string": (str,), "email": (str,), "url": (str,), "text": (str,),
    "enum": (str,), "uuid": (str,), "integer": (int,), "int": (int,),
    "float": (float,), "decimal": (float,), "boolean": (bool,), "bool": (bool,),
...
class FieldTypeCompatibleCompiler:
    ...
    def compile(
                ...
                if default is not None:
                    ...
                    if not isinstance(default, expected):
                        ...
                        assertions.append(NamedAssertion(
                            ...
                            metadata={
                                "message": f"Field '{fname}': {f_type} vs {type(default).__name__}",
        ...
        return assertions or [
            NamedAssertion(name="field_types_ok", expression=z3.BoolVal(True), constraint_id=definition.id)  # noqa: E501
        ]
