...
def test_compilation_settings_defaults():
    ...
    assert s.types_dir == "definitions/types"
    assert s.instances_dir == "instances"
...
def test_lof_settings_composition():
    ...
    assert s.compilation.types_dir == "definitions/types"
    assert s.solver.timeout_ms == 30_000
    assert s.reasoning.default_profile == "fastapi-react"
